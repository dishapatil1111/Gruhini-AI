...
def show_sidebar():
    ...
    if st.sidebar.button("📚 Study Hub", use_container_width=True):
        st.session_state.page = "Study Hub"
        st.rerun()

    if st.sidebar.button("💻 Coding Hub", use_container_width=True):
        st.session_state.page = "Coding Hub"
        st.rerun()

    if st.sidebar.button("🎤 Interview Hub", use_container_width=True):
        st.session_state.page = "Interview Hub"
        st.rerun()

    if st.sidebar.button("💼 Career Hub", use_container_width=True):
    ...
    if st.sidebar.button("➕ New Chat", use_container_width=True):
        st.session_state.chat_id = __import__("uuid").uuid4().hex[:6]
        st.rerun()
